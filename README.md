...
| Designs a change and **stops** — resolves a task ID, explores, settles the open questions with you, writes the spec under that ID, runs nothing. For work whose shape has to be decided before it is scheduled; `task` with the same ID later picks the spec up instead of drafting another. | `/tdd:plan` | `/tdd-plan` |
